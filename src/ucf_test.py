...
def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device):
    ...
    if len(abnormal_video_gt) > 0:
        ...
        ucf_ano_auc = float(roc_auc_score(abnormal_video_gt_arr, abnormal_video_scores_arr))

    print("============================================================")
    print("UCF-Crime Evaluation")
    print("============================================================")
    print("[Coarse-grained anomaly detection - Paper metric]")
    print(f"AUC (All frames)            : {coarse_auc * 100:.2f}%")
    if ucf_ano_auc is not None:
        print(f"Ano-AUC (Abnormal videos)   : {ucf_ano_auc * 100:.2f}%")
    ...
    print("============================================================")
    print(f"UCF abnormal-only evaluation: abnormal_videos={num_abnormal_videos}, frames={len(abnormal_video_scores)}, normal_frames={sum(1 - int(v) for v in abnormal_video_gt)}, abnormal_frames={sum(int(v) for v in abnormal_video_gt)}")
    print("============================================================")

    return coarse_auc, coarse_ap
